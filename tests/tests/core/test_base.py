...
class SampleBase(Base):
    ...
    def format_version(self, value):
        self._validate_format_version(value, strict=True)
    ...
    @classmethod
    def from_dict(cls, data, strict: bool = True) -> "SampleBase":
...
def test_invalid_derived_fields_instantiation(monkeypatch):
    def _derived_attributes(self):
        return ["document_typ"]
    monkeypatch.setattr(SampleBase, "_derived_attributes", _derived_attributes)
...
def test_invalid_data_initialisation(data, exc_type, match, strict):
    if strict:
        with pytest.raises(exc_type, match=re.escape(match)):
            SampleBase.from_dict(data, strict=True)
    else:
        md = SampleBase.from_dict(data, strict=False)
